...
fn main() {
    ...
    println!("{:?}", octree.unwrap().into_iter().map(|tree| tree.depth()).collect::<Vec<u32>>());
    ...
    println!("{:#?}", tree);
    println!("{:?}", AABB::new(1.0, 0.5, 0.75, 1.0, 0.5, 1.0).explode((0.5, 0.5, 0.5)));
    println!("{:?}", AABB::new(0.0, 0.5, 0.0, 1.0, 1.0, 1.0).explode((0.5, 0.5, 0.5)));
}